...
class StatsCounter(object):
    ...
    def __init__(self):
        ...
        max_steps = math.floor(
            self.args.num_samples_per_file
            * self.args.num_files_train
        ...
        if self.args.total_training_steps > 0:
            if self.args.total_training_steps > max_steps:
                logging.error(
                    f"Only have enough data for {max_steps} steps but {self.args.total_training_steps} wanted"
                )
                exit(-1)
    ...
    def end_run(self):
        ...
        if not self.args.generate_only:
            ...
            train_au = (
                np.array(self.comm.allreduce(np.array(self.train_au))) / self.comm.size
            )
            ...
            self.summary["metric"]["train_throughput_samples_per_second"] = list(
                train_throughput
            )
            self.summary["metric"]["train_throughput_mean_samples_per_second"] = (
                np.mean(train_throughput)
            )
            self.summary["metric"]["train_throughput_stdev_samples_per_second"] = (
                np.std(train_throughput)
            )
            self.summary["metric"]["train_io_mean_MB_per_second"] = (
                np.mean(train_throughput) * self.record_size / 1024.0 / 1024.0
            )
            self.summary["metric"]["train_io_stdev_MB_per_second"] = (
                np.std(train_throughput) * self.record_size / 1024.0 / 1024.0
            )
            if self.args.do_eval:
                ...
                self.summary["metric"]["eval_throughput_samples_per_second"] = list(
                    eval_throughput
                )
                self.summary["metric"]["eval_throughput_mean_samples_per_second"] = (
                    np.mean(eval_throughput)
                )
                self.summary["metric"]["eval_throughput_stdev_samples_per_second"] = (
                    np.std(eval_throughput)
                )
                self.summary["metric"]["eval_io_mean_MB_per_second"] = (
                    np.mean(eval_throughput) * self.record_size / 1024.0 / 1024.0
                )
                self.summary["metric"]["eval_io_stdev_MB_per_second"] = (
                    np.std(eval_throughput) * self.record_size / 1024.0 / 1024.0
                )
            if self.my_rank == 0:
                ...
                metric = (
                    metric
                    + f"[METRIC] Training Accelerator Utilization [AU] (%): {np.mean(train_au):.4f} ({np.std(train_au):.4f})\n"
                )
                metric = (
                    metric
                    + f"[METRIC] Training Throughput (samples/second): {np.mean(train_throughput):.4f} ({np.std(train_throughput):.4f})\n"
                )
                metric = (
                    metric
                    + f"[METRIC] Training I/O Throughput (MB/second): {np.mean(train_throughput)*self.record_size/1024/1024:.4f} ({np.std(train_throughput)*self.record_size/1024/1024:.4f})\n"
                )
                metric = (
                    metric
                    + f"[METRIC] train_au_meet_expectation: {self.summary['metric']['train_au_meet_expectation']}\n"
                )

                if self.args.do_eval:
                    metric = (
                        metric
                        + f"[METRIC] Eval Accelerator Utilization [AU] (%): {np.mean(eval_au):.4f} ({np.std(eval_au):.4f})\n"
                    )
                    metric = (
                        metric
                        + f"[METRIC] Eval Throughput (samples/second): {np.mean(eval_throughput):.6f} ({np.std(eval_throughput):.6f})\n"
                    )
                    metric = (
                        metric
                        + f"[METRIC] Eval Throughput (MB/second): {np.mean(eval_throughput)*self.record_size/1024/1024:.6f} ({np.std(eval_throughput)*self.record_size/1024/1024:.6f})\n"
                    )
                    metric = (
                        metric
                        + f"[METRIC] eval_au_meet_expectation: {self.summary['metric']['eval_au_meet_expectation']}\n"
                    )
                metric += "[METRIC] ==========================================================\n"
                logging.info(metric)
    ...
    def start_train(self, epoch):
        if self.my_rank == 0:
            ...
            if self.steps_override:
                logging.info(
                    f"{ts} Starting epoch {epoch}: Overriding number of steps to {self.steps}."
                )
            else:
                logging.info(
                    f"{ts} Starting epoch {epoch}: {self.steps} steps expected"
                )
    ...
    def end_train(self, epoch, steps):
        ...
        throughput = np.array(
            [
                self.output[epoch]["throughput"][k]
                for k in self.output[epoch]["throughput"]
            ]
        )
        steps = np.array(
            [
                len(self.output[epoch]["proc"][k])
                for k in self.output[epoch]["throughput"]
            ]
        )
        if np.sum(steps) == 0:
        ...
        if self.my_rank == 0:
            ts = utcnow()
            duration = pd.to_datetime(ts) - pd.to_datetime(
                self.per_epoch_stats[epoch]["start"]
            )
            duration = "{:.2f}".format(duration.total_seconds())
            self.per_epoch_stats[epoch]["end"] = ts
            self.per_epoch_stats[epoch]["duration"] = duration
            logging.info(
                f"{ts} Ending epoch {epoch} - {np.sum(steps)} steps completed in {duration} s"
            )
    ...
    def start_eval(self, epoch):
        ...
        if self.my_rank == 0:
            ...
            self.per_epoch_stats[epoch]["eval"] = {"start": ts}
    ...
    def end_eval(self, epoch):
        ...
        if self.my_rank == 0:
            ts = utcnow()
            duration = pd.to_datetime(ts) - pd.to_datetime(
                self.per_epoch_stats[epoch]["eval"]["start"]
            )
            duration = "{:.2f}".format(duration.total_seconds())
            logging.info(
                f"{ts} Ending eval - {self.steps_eval} steps completed in {duration} s"
            )
            self.per_epoch_stats[epoch]["eval"]["end"] = ts
            self.per_epoch_stats[epoch]["eval"]["duration"] = duration
            logging.info(
                f"{utcnow()} Epoch {epoch} [Eval] Accelerator Utilization [AU] (%): {self.output[epoch]['au']['eval']:.4f}"
            )
            logging.info(
                f"{utcnow()} Epoch {epoch} [Eval] Throughput (samples/second): {self.output[epoch]['throughput']['eval']*self.comm_size:.4f}"
            )
    ...
    def start_block(self, epoch, block):
        ...
        if self.my_rank == 0:
            ...
            self.per_epoch_stats[epoch][f"block{block}"] = {"start": ts}
    ...
    def end_block(self, epoch, block, steps_taken):
        ...
        if self.my_rank == 0:
            ...
            ts = utcnow()
            duration = pd.to_datetime(ts) - pd.to_datetime(
                self.per_epoch_stats[epoch][f"block{block}"]["start"]
            )
            duration = "{:.2f}".format(duration.total_seconds())
            logging.info(
                f"{ts} Ending block {block} - {steps_taken} steps completed in {duration} s"
            )
            self.per_epoch_stats[epoch][f"block{block}"]["end"] = ts
            self.per_epoch_stats[epoch][f"block{block}"]["duration"] = duration
            logging.info(
                f"{utcnow()} Epoch {epoch} - Block {block} [Training] Accelerator Utilization [AU] (%): {self.output[epoch]['au'][f'block{block}']:.4f}"
            )
            logging.info(
                f"{utcnow()} Epoch {epoch} - Block {block} [Training] Throughput (samples/second): {self.output[epoch]['throughput'][f'block{block}']*self.comm_size:.4f}"
            )
    ...
    def start_ckpt(self, epoch, block, steps_taken):
        if self.my_rank == 0:
            ts = utcnow()
            logging.info(
                f"{ts} Starting checkpoint {block} after total step {steps_taken} for epoch {epoch}"
            )
            self.per_epoch_stats[epoch][f"ckpt{block}"] = {"start": ts}

    def end_ckpt(self, epoch, block):
        if self.my_rank == 0:
            ts = utcnow()
            duration = pd.to_datetime(ts) - pd.to_datetime(
                self.per_epoch_stats[epoch][f"ckpt{block}"]["start"]
            )
            duration = "{:.2f}".format(duration.total_seconds())
    ...
    def batch_loaded(self, epoch, step, block, t0):
        ...
        logging.debug(
            f"{utcnow()} Rank {self.my_rank} step {step}: loaded {self.batch_size} samples in {duration} s"
        )
    ...
    def batch_processed(self, epoch, step, block, t0, computation_time):
        ...
        logging.info(
            f"{utcnow()} Rank {self.my_rank} step {step} processed {self.batch_size} samples in {duration} s"
        )
    ...
    def compute_metrics_train(self, epoch, block):
        ...
        if total_compute_time == 0:
            ...
            total_time = (
                ...
                - self.output[epoch]["proc"][key][0]
            )
            au = total_compute_time / total_time
        throughput = (
            len(self.output[epoch]["compute"][key])
            / (self.end_timestamp - self.start_timestamp)
            * self.batch_size
        )
        logging.info(
            f"bernardhan: len(self.output[epoch]['compute'][key])={len(self.output[epoch]['compute'][key])}, (self.end_timestamp - self.start_timestamp)={(self.end_timestamp - self.start_timestamp)}, self.batch_size={self.batch_size}"
        )
    ...
    def eval_batch_loaded(self, epoch, step, t0):
        ...
        logging.debug(
            f"{utcnow()} Rank {self.my_rank} step {step} loaded {self.batch_size_eval} samples in {duration} s"
        )
    ...
    def eval_batch_processed(self, epoch, step, t0, computation_time):
        ...
        logging.info(
            f"{utcnow()} Rank {self.my_rank} step {step} processed {self.batch_size_eval} samples in {duration} s"
        )
    ...
    def save_data(self):
        ...
        if self.my_rank == 0:
            with open(
                os.path.join(self.output_folder, "per_epoch_stats.json"), "w"
            ) as outfile:
        ...
        with open(
            os.path.join(self.output_folder, f"{self.my_rank}_output.json"), "w"
        ) as outfile:
